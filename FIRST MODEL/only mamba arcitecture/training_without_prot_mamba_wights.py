...
try:
    ...
    class ProtMambaModel(PreTrainedModel):
        """HuggingFace wrapper for the custom Protein Mamba architecture."""
        ...
        def forward(self, input_ids, position_ids=None, **kwargs):
            # Auto-generate position IDs if not provided
...
class HyenaConfig(PretrainedConfig):
    """Configuration class for the HyenaDNA backbone."""
    model_type = "hyenadna"
...
class LatentResampler(nn.Module):
    ...
    def __init__(self, d_model=256, n_latents=512, n_heads=8, dropout=0.1):
        super().__init__()
        # Learnable latent queries that will "summarize" the input sequence
        ...
        # Cross-attention mechanism to map variable length input to fixed latent count
    ...
    def forward(self, x):
        B = x.shape[0]
        # Expand the learnable latents to match the current batch size
        latents = self.latents.unsqueeze(0).expand(B, -1, -1)
        
        # Latents act as Query, while the input sequence (x) acts as Key and Value
        attn_out, _ = self.cross_attn(
            query=self.norm_latents(latents), 
            key=self.norm_ctx(x), 
            value=self.norm_ctx(x)
        )
        
        # Residual connection followed by FFN
...
class FlashMultimodalDecoder(nn.Module):
    ...
    def __init__(self, d_model=256, vocab_size=67, max_len=4096, dropout=0.1, num_species=17):
        ...
        self.layers = nn.ModuleList([
            nn.TransformerDecoderLayer(
                d_model, 8, 1024, batch_first=True, norm_first=True, activation='gelu', dropout=dropout
            ) for _ in range(6)
        ])
        
        #  Change: Initializing 17 separate heads instead of a single global head
        # This allows each species to specialize its own codon bias logic
    ...
    def forward(self, cds_ids, memory, org_ids): 
        #  Added org_ids for routing logic
        B, L = cds_ids.shape
        ...
        # This maps the latent representation to the specific codon distribution of the target organism
        out = torch.zeros(B, L, 67, device=x.device, dtype=x.dtype)
        for i in range(B):
            species_id = org_ids[i].item() # Identify which organism is being processed
...
class OptimizedFusionModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        # ---------------------------------------------------------
        # 1. HYENADNA BACKBONE SETUP
        # ---------------------------------------------------------
        smart_print("🔧 Initializing Hyena...")
        # Configure HyenaDNA for genomic sequence processing
        ...
        # Ensure embedding layer is accessible for token extraction
        ...
        # Enable gradient checkpointing to save VRAM at the cost of a slight compute overhead during the backward pass
        self.hyena.gradient_checkpointing_enable(); self.mamba.gradient_checkpointing_enable()
        
        # ---------------------------------------------------------
        # 3. TEMPORAL COMPRESSION & PROJECTION
        # ---------------------------------------------------------
        # Convolutions to drastically reduce the sequence length (temporal dimension) before Cross-Attention
        self.dna_conv = nn.Conv1d(256, 256, 32, stride=32)
        self.prot_conv = nn.Conv1d(256, 256, 2, stride=2)
        
        # Projection layer to align ProtMamba's 768d output to the shared 256d latent space
        self.prot_proj = nn.Linear(768, 256)
        self.norm = nn.LayerNorm(256)

        # ---------------------------------------------------------
        # 4. EMBEDDINGS & DECODER COMPONENTS
        # ---------------------------------------------------------
        # Differentiates between modality types (0: DNA, 1: Protein)
        self.modal_emb = nn.Embedding(2, 256)
        # Positional embeddings for the compressed amino acid sequence
        self.aa_pos_emb = nn.Embedding(2048, 256)
        # Embeddings for the target organism/species ID
        self.species_emb = nn.Embedding(50, 256)
        
        # Compresses the massive DNA context into a fixed number of latent vectors
        self.resampler = LatentResampler(256, 512, dropout=dropout_val)
        # Generates the final output sequence
        self.cds_decoder = FlashMultimodalDecoder(256, vocab_size=67, max_len=4096, dropout=dropout_val)
        
        # Global classification heads (applied to the averaged memory state)
    ...
    def _apply_lora(self, model, name):
        """Applies Low-Rank Adaptation (LoRA) to efficiently fine-tune the dense backbones without updating all parameters."""
    ...
    def _load_hyena_weights(self):
        """Loads pre-trained weights for the HyenaDNA backbone, mapping keys appropriately."""
    ...
    def _safe_hyena(self, seq):
        """Runs the Hyena backbone safely, zeroing out NaN/Inf values and applying the padding mask."""
    ...
    def _safe_mamba(self, seq):
        """Runs the Mamba backbone safely, zeroing out NaN/Inf values and applying the padding mask."""
    ...
    def forward(self, up, down, introns, aa, org, cds_target=None):
        ...
        # --- Process DNA Modality ---
        # Extract features and compress temporally (Note: PyTorch Conv1d expects [Batch, Channels, Length])
        ...
        # Reshape introns to handle multiple intron sequences per batch item, process, then reshape back
        B, N, L = introns.shape
        e_int = self.dna_conv(self._safe_hyena(introns.reshape(-1, L)).transpose(1,2)).transpose(1,2).reshape(B, -1, 256)

        # Concatenate all DNA context elements and compress them into 512 latent vectors
        dna_ctx = torch.cat([e_up, e_int, e_down], dim=1)
        # Add modality embedding (0) to identify these latents as DNA
        ...
        # Project from 768d down to 256d, then compress temporally
        e_aa = self.prot_conv(self.prot_proj(e_aa).transpose(1,2)).transpose(1,2)
      
        # Generate clamped positional indices for the amino acid sequence
        ...
        # Add positional embedding and modality embedding (1) to identify this as Protein
        e_aa = e_aa + self.aa_pos_emb(pos_indices) + self.modal_emb(torch.ones(1, device=device).long())

        # --- Process Organism ID ---
        org = org.reshape(-1)
        # Clamp organism IDs to prevent out-of-bounds indexing in the embedding layer
        org = torch.clamp(org, min=0, max=self.species_emb.num_embeddings - 1)
        e_species = self.species_emb(org).unsqueeze(1)
        
        # --- Construct Unified Memory Context ---
        # Combine Species ID, DNA Latents, and Protein sequence into a single memory block for the decoder
        memory = torch.cat([e_species, resampled_dna, self.norm(e_aa)], dim=1)
        
        # Return predictions for Organism, Protein Abundance, and the generated CDS tokens
...
def evaluate(model, loader, device, dtype):
    """Evaluates the model over the entire validation loader and computes aggregate metrics."""
    model.eval()
    m = {'loss': 0, 'acc_cds': 0, 'acc_org': 0, 'acc_pa': 0}
    
    with torch.no_grad():
        for b in loader:
            # Move all batch tensors to the target device (GPU)
            for k,v in b.items(): b[k] = v.to(device)
            
            # Use Automatic Mixed Precision (AMP) for faster evaluation
            with torch.amp.autocast('cuda', dtype=dtype):
                p_org, p_pa, p_cds = model(b['up'], b['down'], b['introns'], b['aa'], b['org'], b['cds'][:, :-1])
            
            # Calculate Cross-Entropy Loss (ignoring the padding token '0')
            m['loss'] += F.cross_entropy(p_cds.reshape(-1, 67).float(), b['cds'][:, 1:].reshape(-1), ignore_index=0).item()
            
            # Calculate CDS accuracy by masking out padding tokens to avoid artificially inflating the score
            m['acc_cds'] += (p_cds.argmax(-1) == b['cds'][:, 1:])[b['cds'][:, 1:] != 0].float().mean().item()
            
            # Calculate accuracy for global heads
            m['acc_org'] += (p_org.argmax(-1) == b['org']).float().mean().item()
            m['acc_pa'] += (p_pa.argmax(-1) == b['pa']).float().mean().item()
            
    # Return metrics averaged over the number of batches
...
def main():
    ...
    # Default Training Configuration mapped as a dictionary
    CONFIG = {
    ...
    # Override defaults with custom JSON config if provided via command line args
    ...
    # Check if an interrupted training run exists to resume seamlessly
    ...
    if is_resuming:
        ...
        # Save the current configuration so it can be re-loaded upon failure/resume
        with open(f"{EXPERIMENT_DIR}/config_used.json", "w") as f:
            json.dump(CONFIG, f, indent=4)

    # Initialize Weights & Biases for experiment tracking
    # Passing resume="allow" ensures W&B seamlessly connects to the previous run graph if interrupted
    wandb.init(
    ...
    # Use bfloat16 if hardware supports it (Ampere+ GPUs) to prevent overflow/underflow, else float16
    dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16

    # Setup Datasets and Dataloaders
    train_ds = GenomicDataset(TRAIN_FILE, CONFIG, is_train=True)
    # Use WeightedRandomSampler to balance class distributions during training
    ...
    # Validation loader doesn't need shuffling or sampling
    ...
    # Initialize the architecture and push to the selected precision and device
    ...
    scheduler = get_cosine_schedule_with_warmup(
        ...
        num_training_steps=(len(train_loader) // CONFIG['grad_accum']) * CONFIG['epochs']
    )
    
    # Gradient scaler for Mixed Precision Training (prevents gradients from becoming 0 in float16)
    ...
    if is_resuming:
        try:
            # Load the saved training state to the current device
            state = torch.load(f"{latest_ckpt_path}/training_state.pth", map_location=device)
            
            if 'full_model_state' in state:
                # 1. Extract weights from the previous single-head architecture to reuse them
                # We use .pop() to remove them from the state_dict so they don't cause issues during strict=False loading
                ...
                # 2. Load the backbone and shared model components
                # strict=False is required here because the head architecture has changed from 1 to 17
                model.load_state_dict(state['full_model_state'], strict=False)
                
                # 3. Perform a Warm Start by broadcasting the old head weights to all 17 new parallel species-specific heads
                ...
                # --- 🚀 Handling potential optimizer state mismatches ---
                try:
                    # Attempt to load the optimizer, scheduler, and scaler states
                    optimizer.load_state_dict(state['optimizer'])
                    scheduler.load_state_dict(state['scheduler'])
                    scaler.load_state_dict(state['scaler'])
                    smart_print("✅ Optimizer & Scheduler states loaded.")
                except:
                    # If architecture changes (like adding 17 heads) break the optimizer's internal state, we re-initialize
                ...
                # Sync training progress variables
