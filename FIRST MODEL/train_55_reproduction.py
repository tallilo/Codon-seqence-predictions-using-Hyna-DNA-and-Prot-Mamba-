...
class FlashMultimodalDecoder(nn.Module):
    """Autoregressive Transformer Decoder to generate the final CDS sequence."""
...
class OptimizedFusionModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Manually shrinking dimensions for the randomly initialized Mamba to save compute
        ...
        # Enable gradient checkpointing to save VRAM at the cost of slight compute overhead
        self.hyena.gradient_checkpointing_enable(); self.mamba.gradient_checkpointing_enable()
        
        # Temporal compression blocks (Convolutions) to reduce sequence length
        self.dna_conv = nn.Conv1d(256, 256, 32, stride=32); self.prot_conv = nn.Conv1d(256, 256, 2, stride=2)
        
        # Projection and alignment layers
    ...
    def forward(self, up, down, introns, aa, cds_target=None):
        ...
        # Process DNA contexts through Hyena and compress temporally via Conv1d
        ...
        # Concatenate and resample the DNA sequence into a fixed-size latent representation
        ...
        # Process Protein context through Mamba and align dimensions
        ...
        # Combine DNA latents and Protein sequence into a single memory block for the decoder
        memory = torch.cat([resampled_dna, self.norm(e_aa)], dim=1)
        
        # Return global predictions (Organism, PA) and sequence predictions (CDS)
    ...
    if is_resuming:
        try:
            ...
            if 'full_model_state' in state:
                ...
                scheduler.load_state_dict(state['scheduler']) # Keeps learning rate decay curve consistent
                ...
                smart_print(f"✅ Successfully Resumed! Starting from Epoch {start_epoch}, Step {global_step}, skipping up to Batch {resumed_batch_idx}")
            else:
