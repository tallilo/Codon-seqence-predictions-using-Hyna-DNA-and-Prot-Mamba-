...
# ==========================================
# 🚀 1. CRITICAL CHANGE: IMPORT FROM THE NEW SCRIPT
# ==========================================
# We import from the pre-trained script to get the OptimizedFusionModel that correctly wraps Mamba,
# and to ensure that all HuggingFace registrations (AutoConfig.register) run automatically.
from prot_mamba_pre_trained import OptimizedFusionModel, GenomicDataset, smart_print, ORG_TO_ID, CodonTokenizer
...
# ==========================================
# 2. CRITICAL CHANGE: NEW EXPERIMENT ID
# ==========================================
# Based on the logs, this is the specific experiment name that ran with the pre-trained backbone.
EXPERIMENT_ID = "prot_mamba_pre_trained_32_r" 
...
def run_inference():
    ...
    smart_print(f"🚀 Starting Inference for PRE-TRAINED MAMBA + MULTI-HEADS: {EXPERIMENT_ID}")

    ID_TO_ORG = {v: k for k, v in ORG_TO_ID.items()}
    codon_tok = CodonTokenizer()
    ID_TO_CODON = {v: k for k, v in codon_tok.vocab.items()}
    ID_TO_CODON[0] = ""  # Ignore PAD token
    ID_TO_CODON[66] = "" # Ignore EOS token
    ID_TO_CODON[65] = "<UNK>"

    # Initialize the new model architecture from the configuration file
    model = OptimizedFusionModel(config).to(device).to(dtype)
    
    # Load the fine-tuned weights (LoRA adapters and the specific prediction heads)
    ...
    # ---------------------------------------------------------
    #  HOOK 1: CAPTURE LATENT EMBEDDINGS
    # ---------------------------------------------------------
    saved_embeddings = []
    ...
    # ---------------------------------------------------------
    #  HOOK 2: CAPTURE CROSS-ATTENTION WIEGHTS
    # ---------------------------------------------------------
    saved_attentions = []
    ...
    smart_print(f"✅ Registered hooks on {len(attn_hook_handles)} Attention layers.")
